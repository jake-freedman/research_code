...
def bnc_2d_power_phase_sweep(
    ...
    print(
        f"2D power-phase sweep: {ch1_n_points}×{ch2_n_points} grid, {P} phase steps\n"
    ...
    with BNC855B(BNC_RESOURCE_STRING) as sig, esa_cls(esa_addr) as esa:
        ...
        for i, ch1_pwr in enumerate(ch1_powers):
            ...
            for j, ch2_pwr in enumerate(ch2_powers):
                ...
                print(
                    f"Grid ({i + 1}/{ch1_n_points}, {j + 1}/{ch2_n_points}): "
                ...
                try:
                    ...
                    spectra_arr = np.array(spectra_this)   # (P, N, K)
                    ...
                    np.savez_compressed(
                        ...
                        spectra=spectra_arr,
                        cal_spectra=np.tile(cal_spectrum, (P, 1)),
                    ...
                    print(f"  ERROR: {exc}; skipping grid point ({i}, {j}).")
                    try:
                        sig.disable_all_outputs()
                    except Exception:
                        pass
...
def main():
    ...
    bnc_2d_power_phase_sweep(
        ...
        ch1_power_max_dbm=23,
        ch1_n_points=20,
        ch2_power_min_dbm=-10,
        ch2_power_max_dbm=10,
        ch2_n_points=20,
